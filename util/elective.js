...
const electiveSelector = async (frame) => {
    try {
        const electiveBtn = await frame.waitForSelector("#WD0112-title");
        ...
        for (let y = 0; y < years.length; y++) {
            ...
            for (let s = 0; s < semester.length; s++) {
                ...
                for (let i = 0; i < 1; i++) {
                    ...
                    const rowDown = await frame.waitForSelector("#WD015E-btn");
                    await rowDown.click();
                    await frame.waitForTimeout(500);

                    const rowBtn = await frame.waitForSelector("#WD0165");
                    await rowBtn.click();
                    await frame.waitForTimeout(500);

                    const search = await frame.waitForSelector("#WD021A");
                    await search.click();
